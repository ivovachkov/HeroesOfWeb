    ...
    public class UsersController : BaseApiController
    {
            ...
            [ValueProvider(typeof(HeaderValueProviderFactory<string>))] string sessionKey)
        {
            var response = this.PerformOperationAndHandleExceptions(() =>
            {
                ...
                var users =
                    ...
                    where u.SessionKey != null && u.Busy == false
            ...
            [ValueProvider(typeof(HeaderValueProviderFactory<string>))] string sessionKey)
        {
            var responseMsg = this.PerformOperationAndHandleExceptions(() =>
            {
                ...
                var user = context.Users.FirstOrDefault(a => a.SessionKey == sessionKey);
                if (user == null)
                {
                    throw new InvalidOperationException("Invalid session key");
                }

                UpdateUser(user, model, context);
                context.SaveChanges();

                var response =
                    this.Request.CreateResponse(HttpStatusCode.OK, new UserLoginModel());
                return response;
            });
        ...
        private void UpdateUser(User user, UpdateUserModel model, StoreContext context)
        {
            if (model.Gold < 0)
            {
                throw new ArgumentOutOfRangeException("gold", "Cannot set gold less than 0!");
            }

            user.Gold = model.Gold;
            user.Busy = model.Busy;
        }
